...
Customer ViewCustomerDetails()
{
    ...
    fclose(file);
    attron(COLOR_PAIR(9));
    printw("No reservation found with ID: %ld\n", id);
    attroff(COLOR_PAIR(9));
    Customer emptyCustomer = {0}; // Return an empty customer if not found
...
void cancel(long id, int edit)
{
    ...
    {
        attron(COLOR_PAIR(9));
        printw("Reservation cancelled and file updated successfully.\n");
        attroff(COLOR_PAIR(9));
    }
...
int save()
{
    printw("Do you want to save the edits?\n");
    printw("Press 'c' to cancel or any other key to save\n");
    int choose = getch();
    switch (choose)
    {
    case 'c':
...
void edit()
{
    ...
    int newId = RoomReservation(stat, cust.reservationID, cust.nationalId); // Ensure RoomReservation is defined or included
    if (!save())
    {
        cancel(newId, 1);
        attron(COLOR_PAIR(9));
    ...
    else
    {
        ...
        refresh();
        attron(COLOR_PAIR(9));
        printw("Edits saved successfully\n");
        attroff(COLOR_PAIR(9));
    }
}